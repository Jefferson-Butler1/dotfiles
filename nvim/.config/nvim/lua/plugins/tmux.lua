return {
  ...
  config = function()
    ...
    _G.toggle_float_term = function()
      ...
      if vim.fn.exists(":FloatermToggle") == 2 then
        ...
        if toggle_result == "" then
          ...
          vim.fn.system("tmux has-session -t " .. session_name .. " || tmux new-session -d -s " .. session_name .. " -c " .. vim.fn.getcwd())
          
          -- Open a new Floaterm with the tmux session
          vim.cmd("FloatermNew --name=" .. session_name .. " --title=" .. session_name .. " --height=0.8 --width=0.8 --autoclose=2 tmux attach -t " .. session_name)
        end
      else
    ...
    vim.api.nvim_set_keymap('n', '<leader>t', '<cmd>lua toggle_float_term()<CR>', { noremap = true, silent = true })
